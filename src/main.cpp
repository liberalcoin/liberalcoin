...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 43829); // Liberalcoin: Halves monthly

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.00138889 * 24 * 60 * 60; // Liberalcoin: 2 minutes difficulty update
static const int64 nTargetSpacing = 1 * 60; // Liberalcoin: 1 minute block time
...
{
    // Liberalcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "lol 9:30 PM @ 5 Feb 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Liberalcoin: increase each by adding 2 to bitcoin's value.
